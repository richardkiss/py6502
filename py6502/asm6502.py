...
class asm6502:
    def __init__(self, debug=0):
        ...
        self.text_of_lines = []  # of strings
        self.lines = []  # parsed lines (symbol, opcode, addrmode, value
        self.symbols = []  # of (name,line#) tuples
        ...
        self.object_code = []  # 64 K entries to cover whole memory map
    ...
    def strip_comments(self, thestring):
        self.debug(3, f"string passed to strip_comments()={thestring}")
    ...
    def identify_addressmodeformat(self, remainderstr, linenumber):
        ...
        if thestring == "":
            ...
            self.warning(
                linenumber,
                linetext=remainderstr,
                text=f"Can't make sense of address mode {remainderstr}",
            )
            premode = "nothing"
            value = ""
    ...
    def identify_addressmode(self, opcode, premode, value, linenumber):
        ...
        if (
            (opcode in self.zeropageopcodes)
            and (premode == "number")
            and (self.decode_value(value) != -1)
        ):
            if self.decode_value(value) < 256:
                return "zeropage"
        if (opcode in self.relativeopcodes) and (
            (premode == "number") or (premode == "offset")
        ):
            return "relative"
        ...
        self.debug(
            3,
            f"IDENTIFY_ADDRESSMODE for zeropagex opcode={opcode} premode={premode}",
        )
        ...
        if (opcode in self.zeropagexopcodes) and (premode == "numbercommax"):
            self.debug(
                3,
                "IDENTIFY_ADDRESSMODE (opcode was in self.zeropagexopcodes) and (premode was== numbercommax)",
            )
            self.debug(
                3,
                f"IDENTIFY_ADDRESSMODE decoded value = 0x{self.decode_value(value):x}",
            )
            if self.decode_value(value) < 256:
                return "zeropagex"
        ...
        if (opcode in self.zeropageyopcodes) and (premode == "numbercommay"):
            return "zeropagey"
        if (opcode in self.zeropageindexedindirectxopcodes) and (
            premode == "bracketedindexedx"
        ):
            return "zeropageindexedindirectx"
        if (opcode in self.zeropageindexedindirectyopcodes) and (
            premode == "bracketedcommay"
        ):
    ...
    def decode_extrawords(self, linenumber, linetext, s):
        ...
        for theword in csl:
            ...
            if isinstance(value, int):
                ...
                self.warning(linenumber, linetext, f"Can't parse word string {theword}")
                emptylist = []
                return emptylist
        return newlist
    ...
    def decode_extrabytes(self, linenumber, linetext, s):
        ...
        for i in thelist:
            if isinstance(i, int):
                ...
                self.warning(
                    linenumber, linetext, f"Can't parse byte string {newstring}"
                )
                emptylist = []
                return emptylist
        return newlist
    ...
    def genopcodelist(self):
        self.modeswithlowbytevalue = [
            ...
            "zeropageindexedindirectyabsoluteindexedindirect",
            "zeropageindirect",
            "absoluteindirect",
        ]
    ...
    def build_encoding_table(self):
        ...
        self.otherhexcodes = {}  # Make another list for synonyms
    ...
    def assemble(self, lines):
        ...
        self.instruction_map = [
            None
        ] * 65536  # A map for where the instructions are so the debugger can know
        ...
        for i in range(len(self.allstuff)):
            ...
            if (lowbyte == -1) and (addressmode == "relative"):
                ...
                start = (
                    offset + 2
                )  # Delta is relative to the first byte after the branch instruction
            ...
            if opcode_val is not None:
                ...
                if self.addrmode_length(addressmode) > 0:
                    self.instruction_map[
                        offset + 1
                    ] = -1  # -1 signals the first operand byte
                if self.addrmode_length(addressmode) > 1:
                    self.instruction_map[
                        offset + 2
                    ] = -2  # -2 signals the second operand byte
    ...
    def srecords(self, version, revision, module_name, comment):
        ...
        text = f"{version:02x}{revision:02x}"
        text = text + self.str2asciibytes(module_name + comment)
        addr = "0000"
        addr + text
        length = f"{len(addr + text):02x}"
        ...
        while i < 65536:
            if self.object_code[i] != -1:
                ...
                if i < 65536:
                    ...
                    data = ""
                    for value in values:
                        data = (f"{value:02X}") + data

                    addr = f"{(address >> 8) & 0xFF:02x}{address & 0xFF:02x}"
                    length = f"{len(addr + text):02x}"
        ...
        data = f"{(record_count >> 8) & 0xFF:02x}{record_count & 0xFF:02x}"
        length = "03"
        ...
        length = "03"
        addr = f"{(last_addr >> 8) & 0xFF:02x}{last_addr & 0xFF:02x}"
                ...
                if i < 65536:
                    ...
                    while (nextval != -1) and (localrun < 16):
                        ...
                        if i < 65536:
                            ...
                            nextval = -1
                    length = len(values)
                    astring = f":{length:02X}{address:04x}"
                    astring += datarecord
                    for value in values:
                        astring += f"{value:02X}"
                    ...
                    length = len(values)
                    astring = f"addr={address:04x}  len={length:02x} data="
                    for value in values:
                        astring += f"{value:02X}"
                    theoutput.append(astring)
            else:
