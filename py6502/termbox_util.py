...
class termbox_util:
    ...
    def hline(self, x1, y1, x2):
        ...
        for x in range(x1, x2 + 1):
            self.tb.change_cell(x, y1, ord("─"), self.fg, self.bg)
    ...
    def vline(self, x1, y1, y2):
        ...
        for y in range(y1, y2 + 1):
            self.tb.change_cell(x1, y, ord("│"), self.fg, self.bg)
    ...
    def box(self, x1=0, y1=0, x2="maxx", y2="maxy"):
        ...
        self.tb.change_cell(x1, y1, ord("┌"), self.fg, self.bg)  # top-left corner
        self.tb.change_cell(x2, y1, ord("┐"), self.fg, self.bg)  # top-right corner
        self.tb.change_cell(x1, y2, ord("└"), self.fg, self.bg)  # bottom-left corner
        self.tb.change_cell(x2, y2, ord("┘"), self.fg, self.bg)  # bottom-right corner
    ...
    def add_persistent_viewplane_window(
        ...
        pid = len(self.persistent_vp_list)
        self.persistent_vp_list.append(
            (vp, width, height, srcx, srcy, viewx, viewy, active)
        )
        return pid

    def move_persistent_viewplane_window(self, pid, new_srcx, new_srcy):
        (vp, width, height, srcx, srcy, viewx, viewy, active) = self.persistent_vp_list[
            pid
        ]
        self.persistent_vp_list[pid] = (
            vp,
            width,
            height,
            new_srcx,
    ...
    def edit(self, validator, contents="", max_width=None, presenter=None):
        ...
        while True:
            ...
            if event is not None:
                ...
                elif (tkey == self.tbutil.key_left) or (
                    tkey == self.tbutil.TB_KEY_ARROW_LEFT
                ):
                    # self.tbutil.addstr(self.x,self.y+2," LEFT           ")
                    if cursorpos > 0:
                        cursorpos = cursorpos - 1
                elif (tkey == self.tbutil.key_right) or (
                    tkey == self.tbutil.TB_KEY_ARROW_RIGHT
                ):
                    # self.tbutil.addstr(self.x,self.y+2," RIGHT          ")
                    if cursorpos < (len(contents)):
                        cursorpos = cursorpos + 1
                elif (ttype == 1) and (tkey == 127):  # delete pressed
                    if (cursorpos >= len(contents)) and (
                        cursorpos > 0
                    ):  # cursor is at end. Take one off the end
                        # self.tbutil.addstr(self.x,self.y+2," DELETE END      ")
                        contents = contents[:-1]
                        cursorpos -= 1
                    elif (cursorpos > 0) and (
                        cursorpos < len(contents)
