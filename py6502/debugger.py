...
def draw_commands_view(vptu):
    ...
    vptu.addstr(
        x=1,
        y=1,
        thestring="r: Reset, I: Irq, N:Nmi, s:Step, K:sKip, g: Goto Addr, o/l:page up/down",
        bold=False,
    )
    vptu.addstr(
        x=1, y=2, thestring="ESC: exit B: set Breakpoint M:Modify Memory", bold=False
    )
    ...
    for line in range(startline, endline + 1):
        ...
        for i in range(8):
            ...
            if value >= 0:
                ...
                elif (a.instruction_map[addr + i] >= 0) and (
                    a.instruction_map[addr + i] < 256
                ):
                    vptu.addstr(
                        x=5 + (i * 3),
                        y=line,
            ...
            if (value > 31) and (value < 128):
                vptu.addstr(
                    x=29 + i, y=line, thestring=chr(object_code[addr + i]), bold=False
                )
...
def draw_disassembly_inner_view(vptu, object_code):
    ...
    while (reversecount < 49) and (address > 0):
        if a.instruction_map[address - 1] == -1:
            ...
            address = address - 3
            thetype = "instruction"
        elif (a.instruction_map[address - 1] >= 0) and (
            a.instruction_map[address - 1] < 256
        ):
            address = address - 1
            thetype = "instruction"
        else:
    ...
    while (forwardcount < 50) and (address < 0xFFFF):
        ...
        # An instruction with one operand followed by data
            ...
            (a.instruction_map[address + 1] == -1)
            and (a.instruction_map[address + 2] >= 0)
            and (a.instruction_map[address + 2] < 256)
        ):
            address = address + 2
            thetype = "instruction"
        # An instruction with a 2 byte operand.
            ...
            (a.instruction_map[address + 1] == -1)
            and (a.instruction_map[address + 3] >= 0)
            and (a.instruction_map[address + 3] < 256)
        ):
            address = address + 3
            thetype = "instruction"
        else:
    ...
    for i in range(forwardcount):
        vptu.addstr(
            x=0,
            y=i + reversecount + 1,
            thestring=linelist[i + 1 + reversecount],
            bold=False,
        )
...
def dbg6502(object_code, symbol_table):
    ...
    with termbox.Termbox() as tb:
        ...
        pvid_memory_inner = vptu_memory_outer.add_persistent_viewplane_window(
            vp_memory_inner, leftwidth - 2, memorywindowheight - 2, 0, 0, 1, 1, True
        )
        ...
        while True:
            # Refresh the screen
            (cpdl, dc) = draw_disassembly_inner_view(
                vptu_disassembly_inner, object_code
            )
            # draw_memory_inner_view(vptu_memory_inner, object_code)
            ...
            if type == termbox.EVENT_KEY and key == termbox_util.key_tab:
                ...
                draw_memory_outer_view(vptu_memory_outer, memory_selected)
                draw_disassembly_outer_view(
                    vptu_disassembly_outer, disassembly_selected
                )
                draw_observer_view(vptu_observer, observer_selected, event)
            ...
            # When s is pressed, execute one instruction
                ...
                # Back up a bit in case we are in a multi byte stack op
                    ...
                    draw_memory_inner_view_partial(
                        vptu_memory_inner, startline, endline, object_code
                    )
                    vptu_memory_outer.present()
                elif action == "stack":
                    startline = 0x100
...
parser = argparse.ArgumentParser(
    description="A 65C02 debugger built on the py6502 assembler, diassembler and simulator"
)
...
parser.add_argument(
    "filename", help="read assembly from filename", nargs="?", metavar="FILE"
)
